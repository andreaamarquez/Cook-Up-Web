...
class CAD
{
    ...
    static public function traeNombre($idUsuario)
    {
        $con = new Conexion();
        $query = $con->conectar()->prepare("SELECT nombre FROM usuario WHERE idUsuario = '$idUsuario'");
        if ($query->execute()) {
            $row = $query->fetch(PDO::FETCH_ASSOC);
            return $row;
        } else {
    ...
    static public function traeComentarios($idReceta)
    {
        $con = new Conexion();
        $query = $con->conectar()->prepare("SELECT * FROM comentarios WHERE idReceta = '$idReceta' ORDER BY fecha ASC");
        if($query->execute())
        {
            $datos = [];
            /*Más de un registro */
